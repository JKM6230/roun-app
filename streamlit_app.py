...
# =========================================================
    ...
    if sub_menu == "👥 선수 등록/관리":
        ...
        if not athlete_list:
            ...
            # [Tab 1] 기록 (훈련구분 & 품새 분리 적용)
            with tab1:
                st.subheader(f"📝 {t_name} 훈련 기록")
                with st.form("log"):
                    # [변경] 훈련 구분과 품새를 분리하여 입력
                    c_type, c_poomsae = st.columns(2)
                    train_type = c_type.selectbox("훈련 구분", ["품새 훈련", "기초 체력", "전술/겨루기", "시범"])
                    
                    poomsae_list = ["-", "태극 1장", "태극 2장", "태극 3장", "태극 4장", "태극 5장", "태극 6장", "태극 7장", "태극 8장", 
                                    "고려", "금강", "태백", "평원", "십진", "지태", "천권", "한수", "일여"]
                    poomsae_name = c_poomsae.selectbox("품새 (해당 시 선택)", poomsae_list)
                    if poomsae_name == "-": poomsae_name = ""
                    ...
                    if st.form_submit_button("저장"):
                        try:
                            ...
                            st.success("저장 완료")
                        except: st.error("저장 실패 (시트 헤더를 확인하세요)")
            ...
            with tab2:
                ...
                if uf:
                    ...
                    if st.button("🚀 AI 분석 시작 (초정밀 모드)"):
                        with st.spinner("AI 심판이 프레임 단위로 분석 중입니다..."):
                            try:
                                ...
                                * **[00:xx ~ 00:xx] [동작명]**
                                    * **손/팔:** 예비동작의 크기, 회전, 막는 위치의 정확성
                                    * **발/서기:** 보폭, 무릎 각도, 뒷발 틀어짐 여부
                                    * **중심이동:** 엉덩이 높낮이 변화 여부
                                    * **감점 요인:** (구체적 지적)
